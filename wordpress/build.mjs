...
const files = ['index.html','style.css','hero.css','theme.css','readability.css','app.js','languages.js',
  'assets/VCAC-cover-V5.mp4','assets/congregation.jpg','assets/logo.png','assets/questrial-regular.ttf'];
...
for (const name of files) {
  ...
  if (name === 'index.html') {
    ...
    bytes = Buffer.from('<?php http_response_code(404); exit; ?>\n' + html.replace('<video id="hero-video"', '<video id="hero-video" data-src="assets/VCAC-cover-V5.mp4"'));
  }
  if (name === 'app.js') {
    const js = bytes.toString();
    const old = "heroVideo.src = 'assets/VCAC-cover-V5.mp4'";
    if (!js.includes(old)) throw new Error('Video source changed; review the adapter.');
    bytes = Buffer.from(js.replace(old, 'heroVideo.src = heroVideo.dataset.src'));
  }
...
await fs.copyFile(path.join(source,'THIRD_PARTY_NOTICES.md'),path.join(target,'THIRD_PARTY_NOTICES.md'));
